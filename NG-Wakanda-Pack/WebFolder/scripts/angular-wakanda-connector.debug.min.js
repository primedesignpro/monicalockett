/*! angular-wakanda-connector - v0.0.10 - 2014-04-10 */
(function(exports, global) {
    ...
    WAF.EntityCollection.getEntities = function(pos, howMany, options, userData) {
        ...
        if (pos >= lenFromServer) {
            ...
            if (realHowMany > lenLocal - subpos) realHowMany = lenLocal - subpos;
            if (subpos < lenLocal) for (var i = subpos; i < subpos + realHowMany; ++i) result.push(priv.addedElems[i]);
            WAF.callHandler(false, null, newevent, options, userData);
        } else {
            var realHowMany = howMany;
            if (pos + realHowMany > lenFromServer + lenLocal) realHowMany = lenFromServer + lenLocal - pos;
            var howManyFromServer = realHowMany;
            if (pos + howManyFromServer > lenFromServer) howManyFromServer = lenFromServer - pos;
            priv.fetchData(pos, howManyFromServer, {
                onSuccess: function(event) {
                    ...
                    if (howManyFromServer < realHowMany) {
                        ...
                        for (var i = 0; i < remainHowMany; ++i) result.push(priv.addedElems[i]);
                    }
                    WAF.callHandler(false, null, newevent, options, userData);
                },
    ...
    WAF.EntityCollection.gotEntity = function(cacheInfo, options, userData, position) {
        ...
        var entity;
        if (null != cacheInfo) entity = new WAF.Entity(entityCollection.getDataClass(), cacheInfo.rawEntity); else entity = null;
        var event = {
            entityCollection: entityCollection,
            result: entity,
            entity: entity,
    ...
    WAF.EntityAttributeRelated.setValue = function(relatedEntity) {
        ...
        this.owner.touch();
        this.dataURI = null;
        if (null == relatedEntity) this.relKey = null; else this.relKey = relatedEntity.getKey();
    };
    ...
    wakConnectorModule.factory("wakConnectorService", [ "$q", "$rootScope", "$http", function($q, $rootScope, $http) {
        var ds = null, NgWakEntityClasses = {};
        ...
        var prepare = {
            ...
            wafDataClassAddMetas: function(dataClass) {
                var methodName, dataClassMethods = [], collectionMethods = [], entityMethods = [], attributes;
                for (methodName in dataClass._private.dataClassMethodRefs) if (dataClass._private.dataClassMethodRefs.hasOwnProperty(methodName)) dataClassMethods.push(methodName);
                for (methodName in dataClass._private.entityCollectionMethodRefs) if (dataClass._private.entityCollectionMethodRefs.hasOwnProperty(methodName)) collectionMethods.push(methodName);
                for (methodName in dataClass._private.entityMethodRefs) if (dataClass._private.entityMethodRefs.hasOwnProperty(methodName)) entityMethods.push(methodName);
        ...
        var transform = {
            queryEventToNgWakEntityCollection: function(event, onlyOne) {
                var rawEntities, parsedXhrResponse, userDefinedEntityCollectionMethods, result;
        ...
        var $$create = function(pojo) {
            var dataClassName = this._private.className, ngWakEntity, entity, key, attributes;
            ngWakEntity = new NgWakEntityClasses[dataClassName]();
            if (pojo instanceof WAF.Entity) reccursiveFillNgWakEntityFromEntity(pojo, ngWakEntity, this); else reccursiveFillNgWakEntityFromEntity(pojo, ngWakEntity, this);
            return ngWakEntity;
        };
        var reccursiveFillNgWakEntityFromEntity = function(entity, ngWakEntityNestedObject, currentDataClass) {
            ...
            for (key in attributes) if ("undefined" !== typeof entity[key]) if ("storage" === attributes[key].kind || "calculated" === attributes[key].kind) {
                ...
                } else if (isEntityWafEntity) ngWakEntityNestedObject[key] = entity[key].getValue(); else if ("undefined" !== typeof entity[key]) ngWakEntityNestedObject[key] = isEntityWafEntity ? entity[key].getValue() : entity[key];
            } else if ("relatedEntities" === attributes[key].kind) {
                if (entity[key].__ENTITIES) ngWakEntityNestedObject[key] = transform.jsonResponseToNgWakEntityCollection(attributes[key].getRelatedClass(), entity[key].__ENTITIES); else if (entity[key].__deferred) {
                    ngWakEntityNestedObject[key] = [];
                    ngWakEntityNestedObject[key].$_deferred = {
                        uri: entity[key].__deferred.uri,
                        dataClass: attributes[key].getRelatedClass()
                    };
                    ngWakEntityNestedObject[key].$fetch = function() {
                        console.warn("$fetch on deferred not yet implemented (this one fetches a ollection of entities)");
                    };
                }
                ...
                if (isEntityWafEntity) reccursiveFillNgWakEntityFromEntity(entity[key].relEntity, ngWakEntityNestedObject[key], entity[key].relEntity.getDataClass()); else reccursiveFillNgWakEntityFromEntity(entity[key], ngWakEntityNestedObject[key], ds[ds[currentDataClass.$name].$attr(key).type]);
            }
        };
        ...
        var $$fetch = function(options, mode) {
            ...
            wakOptions.onSuccess = function(event) {
                rootScopeSafeApply(function() {
                    ...
                    } else if ("append" === mode) for (var i = 0; i < event.result.length; i++) {
        ...
        $$toJSON = function() {
            var getCleanObject = function(obj) {
                ...
                if (obj instanceof Array) {
                    ...
                    tmp = {};
                    for (key in obj) if (obj.hasOwnProperty(key) && "$_entity" !== key && "$_deferred" !== key) if (obj[key] instanceof Array || obj[key] instanceof NgWakEntityAbstract) tmp[key] = getCleanObject(obj[key]); else if ("undefined" !== typeof obj[key] && !obj[key].$_deferred) tmp[key] = obj[key];
                }
                return tmp;
            };
        ...
        var $$find = function(options) {
            ...
            if (!options || "object" !== typeof options) throw new Error("Please pass an object as options");
            ...
            if (typeof options.pages) wakOptions.pages = options.pages;
        ...
        var NgWakEntityAbstractPrototype = {
            ...
            $syncEntityToPojo: function() {
                var pojo = this, key;
                if (pojo.$_entity && pojo.$_entity._private && pojo.$_entity._private.values) for (key in pojo.$_entity._private.values) {
                    console.log(key, pojo.$_entity._private.values[key]);
                    if (pojo.$_entity[key].getValue() !== pojo[key] && pojo.$_entity[key] instanceof WAF.EntityAttributeSimple) pojo[key] = pojo.$_entity[key].getValue();
                }
...
//# sourceMappingURL=angular-wakanda-connector.debug.min.js.map